...
def step(
    data: np.ndarray, transition: np.ndarray, emission: np.ndarray, initial: np.ndarray
):
    # calculate forward and backward & casting them into numpy arrays
    forward, backward, norm = get_forward_backward_likelihood(
        data, initial, transition, emission
    )

    # calculate the temporary variables, hidden state prob and transition prob
    hidden_state_prob, transition_prob = estimate_hidden_transition(
        data, forward, backward, transition, emission, norm
    )

    # updated variables - transition, emission, and initial
    initial, transition, emission = update_variables(
        data, hidden_state_prob, transition_prob, emission
    )
...
def baum_welch(
    ...
    for _ in range(niters):
        transition, emission, initial, likelihood = step(
